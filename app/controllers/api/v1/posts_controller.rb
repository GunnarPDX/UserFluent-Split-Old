  ...
  module V1
    class PostsController < ApplicationController
      ...
      def index
        ...
        # render json:  @pagy, @posts_w_liked, each_serializer: PostsIndexSerializer
        render json: {
      ...
      def content_filter # ADD RANK PARAM
        if params[:platform].present?
          result = Post.search(params[:query]).where(platform: params[:platform])
        else
          result = Post.search(params[:query])
        end
        result
      end
      ...
      def find_data_for_posts
        ...
        @pagy, posts = pagy(search, page: params[:page], items: 20)
        # posts = Post.all
        posts.each do |p|
